  ...
  <div class="min-h-screen bg-[color:var(--color-bg)] text-[color:var(--color-ink)] antialiased">
    ...
    <div v-if="showNotice" class="fixed top-0 left-0 right-0 z-[60] bg-[#0A0B0E] text-white">
      <div class="max-w-[1280px] mx-auto px-4 lg:px-10 h-9 flex items-center justify-center gap-1.5 sm:gap-2 text-[11px] sm:text-[13px] relative">
        ...
        <template v-else-if="noticeVariant === 'discord'">
          <span class="text-[#5865F2] hidden sm:inline">
            <svg class="w-3.5 h-3.5 inline align-[-2px]" fill="currentColor" viewBox="0 0 24 24"><path d="M20.317 4.37a19.791 19.791 0 00-4.885-1.515.074.074 0 00-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 00-5.487 0 12.64 12.64 0 00-.617-1.25.077.077 0 00-.079-.037A19.736 19.736 0 003.677 4.37a.07.07 0 00-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 00.031.057 19.9 19.9 0 005.993 3.03.078.078 0 00.084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 00-.041-.106 13.107 13.107 0 01-1.872-.892.077.077 0 01-.008-.128 10.2 10.2 0 00.372-.292.074.074 0 01.077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 01.078.01c.12.098.246.198.373.292a.077.077 0 01-.006.127 12.299 12.299 0 01-1.873.892.077.077 0 00-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 00.084.028 19.839 19.839 0 006.002-3.03.077.077 0 00.032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 00-.031-.03z"/></svg>
          </span>
          <span class="text-white/70 truncate min-w-0"><span class="hidden sm:inline">Deals, price drops, and </span><span class="sm:hidden">Join our </span><strong class="text-white font-semibold">Discord</strong><span class="hidden sm:inline"> — live in our community.</span></span>
          <a href="https://discord.gg/uYj2M9XKa5" target="_blank" rel="noopener" class="font-semibold text-[color:var(--color-accent-400)] hover:text-white transition-colors flex-shrink-0">Join →</a>
        </template>
        <template v-else>
          <span class="text-white/50 hidden sm:inline">🚀</span>
      ...
      <div class="border-t border-white/[0.06]">
        <div class="max-w-[1280px] mx-auto px-5 lg:px-10 py-5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p class="text-[12px] text-white/20 ui-mono">
            © {{ currentYear }} Peptidemap
          </p>
          <div class="flex items-center gap-4">
            <div class="flex items-center gap-3">
              <a href="https://www.instagram.com/peptide.map/" target="_blank" rel="noopener" class="text-white/30 hover:text-white/70 transition-colors" aria-label="Instagram"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg></a>
              <a href="https://www.facebook.com/peptidemap/" target="_blank" rel="noopener" class="text-white/30 hover:text-white/70 transition-colors" aria-label="Facebook"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>
              <a href="https://discord.gg/uYj2M9XKa5" target="_blank" rel="noopener" class="text-white/30 hover:text-white/70 transition-colors" aria-label="Discord"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.317 4.37a19.791 19.791 0 00-4.885-1.515.074.074 0 00-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 00-5.487 0 12.64 12.64 0 00-.617-1.25.077.077 0 00-.079-.037A19.736 19.736 0 003.677 4.37a.07.07 0 00-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 00.031.057 19.9 19.9 0 005.993 3.03.078.078 0 00.084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 00-.041-.106 13.107 13.107 0 01-1.872-.892.077.077 0 01-.008-.128 10.2 10.2 0 00.372-.292.074.074 0 01.077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 01.078.01c.12.098.246.198.373.292a.077.077 0 01-.006.127 12.299 12.299 0 01-1.873.892.077.077 0 00-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 00.084.028 19.839 19.839 0 006.002-3.03.077.077 0 00.032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 00-.031-.03z"/></svg></a>
            </div>
    ...
    <!-- Discord join toast — bottom-right after 25s of dwell, dismissible.
         Doesn't render on demo host or if the user dismissed within 30 days. -->
    <transition
    ...
    >
      <div v-if="showDiscordToast" class="fixed bottom-4 right-4 z-[70] w-[calc(100vw-2rem)] sm:w-[340px] max-w-sm">
        <div class="bg-[#0A0B0E] border border-white/[0.08] rounded-[14px] shadow-[0_20px_50px_rgba(0,0,0,0.35)] p-4 relative">
          <button @click="dismissDiscordToast" aria-label="Dismiss" class="absolute top-2 right-2 text-white/30 hover:text-white/70 transition-colors p-1">
            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18M6 6l12 12"/></svg>
          </button>
          <div class="flex items-start gap-3 pr-6">
            <div class="flex-shrink-0 w-10 h-10 rounded-[10px] bg-[#5865F2]/15 border border-[#5865F2]/30 flex items-center justify-center">
            ...
            <div class="min-w-0 flex-1">
              <div class="text-[14px] font-semibold text-white">Join the Peptidemap Discord</div>
              <div class="text-[12px] text-white/60 mt-0.5 leading-snug">Live price-drop alerts, vendor reviews, and a daily peptide spotlight — free.</div>
              <a
                href="https://discord.gg/uYj2M9XKa5"
                target="_blank"
                rel="noopener"
                @click="dismissDiscordToast"
                class="inline-flex items-center gap-1.5 mt-3 h-8 px-3 bg-[#5865F2] hover:bg-[#4752c4] text-white text-[12px] font-semibold rounded-[8px] transition-colors"
              >
                Join server
                <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
              </a>
            </div>
          </div>
        </div>
...
// Rotate notice-bar CTA per session so Discord + Get-Listed both get airtime.
// Persisted in sessionStorage so a user doesn't see it flip on every nav.
const noticeVariant = ref('beta')
if (typeof window !== 'undefined') {
  let v = sessionStorage.getItem('pmap.notice')
  if (!v) {
    v = Math.random() < 0.5 ? 'beta' : 'discord'
    sessionStorage.setItem('pmap.notice', v)
  }
  noticeVariant.value = v
}

// Discord join toast — appears bottom-right after 25s on any given session,
// dismissible, remembers dismissal for 30 days so we don't nag returners.
const showDiscordToast = ref(false)
if (typeof window !== 'undefined') {
  const dismissedAt = Number(localStorage.getItem('pmap.discord-toast.dismissed') || 0)
  const THIRTY_DAYS = 30 * 86400 * 1000
  if (Date.now() - dismissedAt > THIRTY_DAYS && !isDemoHost.value) {
    setTimeout(() => { showDiscordToast.value = true }, 25000)
  }
}
function dismissDiscordToast() {
  showDiscordToast.value = false
  try { localStorage.setItem('pmap.discord-toast.dismissed', String(Date.now())) } catch {}
}
